...
def _log_kraken_result_structure(result) -> None:
    ...
    Kraken 5.x retourne un objet Segmentation avec :
      - result.type         : 'baselines'
      - result.imagename    : pathlib.Path
      - result.text_direction
      - result.script_detection
      - result.lines        : list[Baseline]  ← contient les lignes !
      - result.regions      : dict{str: list[list[tuple]]}  ← régions par type

    Chaque Baseline a :
      - .baseline  : list[tuple(x, y)]
      - .boundary  : list[tuple(x, y)]  ← polygone de contour
      - .tags      : dict{'type': 'default'} ou None

    Affichage limité aux 2 premières lignes pour ne pas polluer les logs.
    """
    ...
    if hasattr(result, 'regions'):
        if isinstance(result.regions, dict):
            ...
            # Compter les polygones de régions
...
class KrakenSegmenter:
    ...
    def segment_page(
        ...
        try:
            # kraken.blla.segment retourne un objet Segmentation (Kraken 5.x)
            result = blla.segment(img, model=self.model, text_direction=text_direction)

            # DEBUG : inspecter la structure de l'objet retourné (utile en dev)
    ...
    def _parse_kraken_result(self, result, page_id: str, img_w: int, img_h: int) -> Tuple[List[PageRegion], List[LinePolygon]]:
        ...
        Kraken 5.x retourne un objet `Segmentation` avec :
          - result.regions : dict {region_type_str: [list of boundary polygons (list of (x,y))]}
          - result.lines   : list de Baseline objects avec .baseline et .boundary

        Kraken <= 4.x retournait une liste d'objets avec .lines et .boundary.
        Ce parser gère les deux formats.
        """
        regions = []
        lines = []
        line_idx = 0

        # ── Kraken 5.x : result.regions est un dict ──────────────────────────
        if hasattr(result, 'regions') and isinstance(result.regions, dict):
            for r_idx, (region_type, region_polys) in enumerate(result.regions.items()):
                ...
                for poly_raw in region_polys:
                    # poly_raw est une liste de tuples (x, y)
                    ...
                    page_region = PageRegion(
                        ...
                        polygon=region_poly,
                        bbox=region_bbox,
                        lines=[],  # lignes assignées ci-dessous
                        confidence=0.9
                    )
                    regions.append(page_region)

        # ── Lignes : result.lines est toujours une liste dans Kraken 5.x ─────
        if hasattr(result, 'lines') and result.lines:
            for line in result.lines:
                ...
                # Déterminer le type de région de la ligne
                # Kraken 5.x : tags = {'type': [{'type': 'default'}]} (liste de dicts)
                #              ou    : {'type': 'default'} (dict simple)
                region_type = "text"
                if hasattr(line, 'tags') and line.tags:
                    if isinstance(line.tags, dict):
                        raw = line.tags.get('type', 'text')
                        # raw peut être une liste de dicts : [{'type': 'default'}]
                        if isinstance(raw, list):
                            if raw and isinstance(raw[0], dict):
                                raw = raw[0].get('type', 'text')
                            elif raw and isinstance(raw[0], str):
                                raw = raw[0]
                            else:
                                raw = 'text'
                        region_type = str(raw) if raw else 'text'
                    else:
                        region_type = str(line.tags)

                # Normaliser les types Kraken vers les types du data contract
                _KRAKEN_TYPE_MAP = {
                    'default': 'main_text', 'text': 'main_text',
                    'marginalia': 'marginalia', 'margin': 'marginalia',
                    'heading': 'heading', 'title': 'heading',
                    'illustration': 'illustration', 'drop_capital': 'main_text',
                }
                region_type = _KRAKEN_TYPE_MAP.get(region_type.lower(), 'main_text')
                ...
                line_obj = LinePolygon(
                    ...
                    reading_order=line_idx,  # Sera recalculé par _compute_reading_order
                    confidence=float(confidence)
                )
                lines.append(line_obj)
                line_idx += 1

        # ── Fallback Kraken <= 4.x : result.regions est une liste d'objets ──
        ...
        # Créer une région synthétique si aucune région n'a été détectée
    ...
    def _extract_polygon(self, obj, img_w: int, img_h: int) -> List[List[int]]:
        ...
        Kraken 5.x : les objets Baseline ont .boundary (liste de tuples (x,y))
                     ou éventuellement .envelope selon la version.
        Kraken <= 4.x : idem mais parfois .bbox uniquement.
        """
        # Kraken 5.x Baseline object : attribut boundary
        ...
        # Kraken 5.x : certains objets exposent .envelope
        ...
        # Dernier recours : polygone couvrant toute l'image
    ...
    def _extract_baseline(self, obj, img_w: int, img_h: int) -> List[List[int]]:
        ...
        Kraken 5.x : obj.baseline est une liste de tuples (x, y).
        """
...
def visualize_segmentation(
    ...
    def _safe_color(region_type):
        """Retourne la couleur même si region_type est une liste ou un type inattendu."""
        if isinstance(region_type, list):
            # Ex: [{'type': 'default'}] → extraire la valeur
